...
export class PowershellService {
  ...
  public get<T>(cmd: PowerShellCommand, reviver?: (this: any, key: string, value: any) => any): Observable<T> {
    const scriptName = cmd.script.split('\n')[0];
...
Parsing error: ${rtnObject.errorsReported}`);
  ...
  public invoke(scriptName: string, cmd: PowerShellCommand): Observable<any[]> {
    return this.session.powerShell.run(cmd).pipe(
        instrument(logSource, `${scriptName}(${JSON.stringify(cmd.parameters)})`),
